...
- легкость понимания для новых разработчиков -- у нихъ не будет трудностей с чтением всех функций `formuploader`
...
Есть различные типы ошибок: синтаксические ошибки вызванные программистом (обычно выявляются при при первом запуске программы), ошибки во время исполнения (код начал выполняться, но какая-то ошибка всё испортила), неверное расширение файла, сбой в работе жесткого диска, отсутствие интернет соединения и т.д.
...
```
Первый аргумент `error` не даст вам забыть обработать свои ошибки. Если бы это был второй аргумент, то вы могли бы написать код как `function handleFile (file) { }` и легко пропустить ошибку.

Линтеры кода также можно настроить так, чтобы помочь вам не забывать обрабатывать ошибки при обратных вызовах. Самый простой для использования -- [стандарт](http://standardjs.com/). Все, что вам нужно сделать, это запустить `$ standard` в своей папке с кодом и он покажет вам каждый обратный вызов с необработанной ошибкой.
...
Самый важный аспект для избавления от ада обратных вызовов -- **перемещение функций за пределы пути** так, чтобы поток выполнения программы был более понятен для чужих разработчиков, которым необходимо будет пробираться через все функции, чтобы понять суть того, что программа пытается сделать.
