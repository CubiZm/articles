...
Давайте возьмем шаблонный код сверху и превратим его в модуль, разделив на несколько файлов. Я покажу модульный паттерн, который работает с любым браузерным или серверным кодом (или работаем и там, и там). Назовем наш новый файл `"formuploader.js"`и он содержит в себе две фунции:
...
`module.exports` это небольшой пример модуля из системы node.js который работает на node, Electron и при этом браузер использует [browserify](https://github.com/substack/node-browserify). Мне очень нравится этот стиль для модулей так как он работает везде, очень легок в понимании и не требует сложных конфигурационных файлов и скриптов.

Теперь, когда у нас есть `formuploader.js` (и он будет загружен на страницу в тег script после запуска browserified) нам просто нужно потребовать и использовать его!
...
Есть различные типы ошибок: синтаксические ошибки (обычно выявляются при при первом запуске программы), ошибки во время исполнения (код начал выполняться, но какая-то ошибка всё испортила), неверное расширение файла, сбой в работе жесткого диска, отсутствие интернет соединения и т.д.

Первые два правила подтверждают не только то, что ваш код должен быть легкочитаемым, это так же делает код устойчивым к ошабкам. Когда мы имеем дело с обратными вызовами, мы по определению имеем дело с задачами, которые были отправились на выполнение, запустились, сделали что-то в фоновом режиме и успешно завершились или прервались из-за ошибки. Любой опытный разработчик скажет вам, что вы никогда не сможете предугадать когда эти ошибки произойдут, поэтому вы всегда должны обрабатывать ошибки для предотвращения таких случаев.
...
Так же можно настроить линтеры кода, чтобы помочь вам не забывать обрабатывать ошибки при обратных вызовах. Самый простой для использования -- [стандарт](http://standardjs.com/). Все что вам нужно сделать -- это запустить `$ standard` в своей папке с кодом и он покажет вам каждый обратный вызов с необработанной ошибкой.

## Краткий итог

1. Не вкладывайте функции. Дайте им имена и разместите их на верхнем уровне программы
2. Используйте [поднятие функций(function hoisting)](https://gist.github.com/maxogden/4bed247d9852de93c94c) для перемещении функций 'ниже сгиба'.
3. Обрабатывайте **каждую ошибку** в каждом вашем обратном вызове. Используйте линтер [стандарт](http://standardjs.com/) для этого.
...
Самый важный аспект для избавления от ада обратных вызовов -- **перемещение функций за пределы пути** так, чтобы поток выполнения программы был более понятен для чужих разработчиков, которым необходимо будет разбирать все функции, чтобы понять суть того, что программа пытается сделать.
...
- Если у вас есть некоторый код, который может быть использован в нескольких проектах, создайте ему собственный readme, тест, `package.json` и опубликуйте его на github и npm. У этого подхода есть много преимуществ, но мы не будем перечислять их здесь
...
Так же прочтите [карманный справочник browserify](https://github.com/substack/browserify-handbook) в качестве примера как писать модульный код.
...
**Генераторы** позволяют вам 'приостановить' отдельную функцию без приостановки всей программы, которые за счет чуть более сложного кода позволяют вашему асинхронному коду выполняться в стиле сверху-вниз. Посмотрите [watt](https://github.com/mappum/watt) для примера такого подхода.

**Асинхронные функции** -- предложенная особенность ES7, которая обернет генераторы и промисы в высокоуровневый синтаксис. Рассмотрите их, если это звучит интересно для вас.

Лично я использую обратные вызовы в 90% случаях когда пишу асинхронный код и, когда все становится сложно, я обращаюсь к [run-parallel](https://github.com/feross/run-parallel) или [run-series](https://github.com/feross/run-series). Я не думаю, что обратные вызовы или промисов или все остальное действительно имеет значение для меня,так как мне важно сохранение кода простым, не вложенным и разделенным на маленькие модули.
